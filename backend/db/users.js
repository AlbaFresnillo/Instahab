...
// Devuelve la información pública de un usuario por su correo electrónico
const getUserByEmail = async (email) => {
  ...
  try {
    ...
    if (result.length === 0) {
      throw generateError('No hay ningún usuario con ese correo electrónico', 404);
    }
...
const createUser = async (email, password) => {
  ...
  try {
    connection = await getConnection();
    // Comprobar que no exista otro usuario con ese correo electrónico
    ...
    if (user.length > 0) {
      throw generateError(
        'Ya existe un usuario en la base de datos con ese correo electrónico',
        409
      );
    }
